...
function Winwheel(options, drawWheel)
{
    defaultOptions = {
        ...
        'drawMode'          : 'image',       // The draw mode. Possible values are 'code', 'image', 'segmentImage'. Default is code which means segments are drawn using canvas arc() function.
...
Winwheel.prototype.draw = function(clearTheCanvas)
{
    ...
    if (this.ctx)
    {
        ...
        if (this.pointerGuide.display == true)
        {
            ...
            var imagePique = document.getElementById('piqueRoue');
            this.ctx.drawImage(imagePique, 364, 50, 40, 70);
        }
    }
}
...
//         var img = document.getElementById("piqueRoue").getAttribute('../img/pique.png');
//         // ctx.drawImage(img, 354, 50, 60, 100);

//         // Rotate and then draw the wheel.
//         // We must rotate by the rotationAngle before drawing to ensure that image wheels will spin.
//         this.ctx.save();
//         this.ctx.translate(this.centerX, this.centerY);
//         this.ctx.drawImage(img, 354, 50, 60, 100);
...
{
    defaultOptions = {
        'visible'        : false,     // In future there might be some functionality related to the pins even if they are not displayed.
...
function Animation(options)
{
    ...
    defaultOptions = {
        'type'              : 'spinToStop',   // For now there are only supported types are spinOngoing (continuous), spinToStop, spinAndBack, custom.
        ...
        'soundTrigger'      : 'pin'             // Sound trigger type. Default is segment which triggers when segment changes, can be pin if to trigger when pin passes the pointer.
    };
...
function Segment(options)
{
    ...
    defaultOptions = {
        ...
        'WinResult'         : true //Determinate if player win or not.
    };
